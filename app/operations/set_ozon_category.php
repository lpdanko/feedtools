...
function op_set_ozon_category(array $cfg, array $ds, int $opId, array $params, callable $log): array
{
  $datasetId = (int)$ds['id'];
  ...
  $newVal = trim((string)($params['ozon_category'] ?? ''));
  if (!$clearCategory && $newVal === '') {
    throw new RuntimeException("Param ozon_category is required");
  }
  ...
  $log("set_ozon_category: mode=" . ($clearCategory ? 'clear' : 'assign') . " value={$newVal}\n");
  ...
  // when we replace <ozon_category>, ignore the rest of the original node subtree
  $skipNode = null;
  ...
  while ($reader->read()) {
    ...
    switch ($reader->nodeType) {
      ...
      case XMLReader::ELEMENT: {
        ...
        if ($name === 'offer') {
          ...
          if ($reader->isEmptyElement) {
            if ($targetOffer && !$clearCategory) {
              $writer->writeElement('ozon_category', $newVal);
              $insertedNew++;
            }
            $writer->endElement();
            $inOffer = false;
        ...
        // replace existing ozon_category in targeted offers
        if ($inOffer && $targetOffer && $name === 'ozon_category') {
          $ozonSeen = true;
          $updatedExisting++;
      ...
      case XMLReader::END_ELEMENT: {
        ...
        if ($name === 'offer' && $inOffer && $reader->depth === $offerDepth) {
          if ($targetOffer && !$ozonSeen && !$clearCategory) {
            $writer->writeElement('ozon_category', $newVal);
            $insertedNew++;
          }
  ...
  if ($inplace) {
    ...
    // 1) sha256 of result, check duplicates (prevent sha256 unique collisions with other datasets)
    ...
    // 2) backup original xml into operation outputs for manual rollback
    $backupAbs = $outDir . '/backup_before_inplace.xml';
    @copy($dstAbs, $backupAbs);

    // 3) atomic replace: write tmp in same directory and rename
    ...
    // 4) rescan and update dataset row
    ...
    // optional: register derivation entry (still useful for audit)
    $ins = db()->prepare("
  ...
  // auto create derived dataset (optional) — копия механики из gpt_fix_title_ru
